...
LLM stack: **OpenAI as primary, Google Gemini as fallback**. Both clients are pinned in [backend/requirements.txt](backend/requirements.txt) (`langchain-openai`, `langchain-google-genai`). This is a deviation from PRD §3.C — see [docs/phases.md](docs/phases.md) Phase 0 for the trade-off note.
...
- **P0-ML-002 — OpenAI implementation (primary)**
  - Area: ML
  - Depends on: P0-ML-001
  - Acceptance: Wraps `langchain-openai` ChatOpenAI; supports streaming; respects `temperature`, `max_tokens`; surfaces token usage on completion.

- **P0-ML-003 — Gemini implementation (fallback)**
  - Area: ML
  - Depends on: P0-ML-001
  - Acceptance: Wraps `langchain-google-genai`; same interface as OpenAI client; integration test against a real key in CI on a small prompt.

- **P0-ML-004 — Provider-selection wrapper**
  - Area: ML
  - Depends on: P0-ML-002, P0-ML-003
  - Acceptance: Single `get_llm()` factory returns OpenAI by default, Gemini when `LLM_PROVIDER=gemini`, or auto-fallback on configured exceptions; logs which provider served each request.
...
- **P0-ML-006 — Embedding model loader**
  - Area: ML
  - PRD: §4
  - Acceptance: `BGE-M3` chosen and documented in `docs/adr/0002-embedding-model.md`; loader caches the model in-process; benchmark on 100 sample docs recorded.
...
- **P1-ML-006 — Hybrid retrieval with re-ranking**
  - Area: ML
  - Acceptance: Dense (BGE-M3) + keyword (BM25) retrieval combined; cross-encoder re-ranker on top-K; latency budget for retrieval ≤ 600ms recorded.
...
- **P9-ML-002 — Provider circuit breaker (OpenAI ↔ Gemini)**
  - Area: ML
  - Depends on: P0-ML-004
  - Acceptance: Opens after N consecutive 429s/5xx or sustained latency breach; half-open probe restores when healthy; chaos test forcing OpenAI failures recovers within SLA.
